      ...
      <CardContent>
        <div className="space-y-2">
          {sortedSessions.map((session, index) => {
            ...
            const hasHRChart =
              session.has_hr && session.hr_samples && session.hr_samples.length > 0
            const isSelected =
              selectedSession != null &&
              selectedSession.start_time === session.start_time
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {session.has_laps && session.num_laps && (
                    <Badge variant="outline" className="gap-1 text-indigo-600 border-indigo-200">
                      ⚡ {session.num_laps} laps
                    </Badge>
                  )}
                  {!session.parseable && (
                    <Badge variant="destructive">Datos básicos</Badge>
                  )}
                </div>
