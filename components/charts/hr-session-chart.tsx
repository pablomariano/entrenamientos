...
const HR_ZONES = [
  ...
  { zone: 2, label: "Z2", min: 100, max: 130, fill: "#10b981", opacity: 0.10 },
  { zone: 3, label: "Z3", min: 130, max: 155, fill: "#f59e0b", opacity: 0.10 },
...
function getLapSeparators(laps: Lap[]): LapSeparator[] {
  ...
  if (laps[0]?.time_seconds !== undefined)
    return laps.filter((l) => (l.time_seconds ?? 0) > 0).map((l) => ({
  ...
  if (laps[0]?.approximate_time_seconds !== undefined)
    return laps.filter((l) => (l.approximate_time_seconds ?? 0) > 0).map((l) => ({
      time_seconds: l.approximate_time_seconds!,
      lap_number: l.lap_number,
    }))
  return []
}
...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: any[] }) {
  ...
  return (
    <div className="bg-popover border border-border rounded-lg px-3 py-2 text-xs shadow-md">
      ...
      <p className="font-bold" style={{ color: zone.fill }}>
        {hr} bpm — {zone.label}
      </p>
    </div>
  )
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground text-center py-6">
            Sin muestras de frecuencia cardíaca disponibles.
          </p>
        </CardContent>
      </Card>
    )
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            {lapSeparators.map((lap, i) => {
              ...
              return (
                <span
                  key={lap.lap_number}
                  className="inline-flex items-center gap-1 text-xs bg-primary/10 text-primary border border-primary/20 rounded-full px-2 py-0.5"
                >
