...
import { TrainingSession, HRSample } from "@/lib/data-processor"
...
function computeZones(samples: HRSample[]): ZoneData[] {
  ...
  // Tiempo por zona usando intervalos entre muestras consecutivas
  ...
  for (let i = 0; i < valid.length; i++) {
    ...
    const zoneIdx = HR_ZONES.findIndex(
      (z) => hr >= z.min && hr < z.max
    )
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground text-center py-6">
            Sin datos de frecuencia cardíaca disponibles.
          </p>
        </CardContent>
      </Card>
    )
          ...
          {zones.map((z) =>
            z.percentage > 0 ? (
              <div
                ...
                title={`${z.label}: ${formatSeconds(z.seconds)}`}
              />
            ) : null
          )}
        </div>
        ...
        <div className="space-y-2">
          {zones.map((z) => (
            <div key={z.zone} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-xs text-muted-foreground/70 tabular-nums w-24 text-right">
                {z.range}
              </span>
